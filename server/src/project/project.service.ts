...
export class ProjectService {
  ...
  async create(createProjectDto: CreateProjectDto,admainId: string,): Promise<any> {
    const newProject = {
      ...createProjectDto,
      admain: admainId,
      createdAt: new Date(),
    ...
    let createProject  = null;
     await this.db.transaction(async (session) => {
      ...
      createProject = new this.projectModel(newProject,{session: session});
      ...
      const userDoc = await this.userService.addProject(createdProjectId,admainId,'ADMAIN',null);
